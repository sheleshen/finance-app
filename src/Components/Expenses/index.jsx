...
const Expenses = () => {
  ...
  let totalExpense = expenses.reduce(
    (curretSum, currentItem) => curretSum + parseInt(currentItem.spending),
    0,
  );
        ...
        {expenses.map((expense) => (
          <div className="flex justify-between pb-3">
            <div className="flex gap-5 items-center">
              ...
              <p className="text-3xl font-bold">{expense.category}</p>
            </div>
            <p className="text-3xl">- {expense.spending} руб.</p>
          </div>
        ))}
      </div>
    </div>
